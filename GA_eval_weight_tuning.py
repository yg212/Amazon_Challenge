...
def generate_initial_population(zone_groups, data_travel, population_size):
    ...
    for _ in range(population_size):
        num_good_start = 5
        count_initial_sol = 0
        if count_initial_sol <= num_good_start:
            all_zones = nearest_zone_sequence(zone_groups, data_travel, similarity_weight=0.5)
            count_initial_sol+=1
        else:
            random.shuffle(all_zones)  # Shuffle zones for diversity in solutions
...
def evaluate_population(population, data_travel, precedence_matrix, station_code, similarity_weight, travel_weight):
    evaluations = []
    ...
    for individual in population:
        ...
        similarity_fitness = (total_similarity + history_similarity)*similarity_weight
        fitness = similarity_fitness - (total_travel_length )*travel_weight

        evaluations.append((fitness, similarity_fitness , total_travel_length))
...
def intra_zone_crossover(parent1, parent2):
...
def Main(data_route, data_travel, population_size, num_generations, mutation_rate, crossover_rate, time_limit, debug, similarity_weight, travel_weight):
    ...
    for generation in range(num_generations):
        ...
        fitness_scores = evaluate_population(population, data_travel, filtered_precedence_matrix_by_station, station_code, similarity_weight, travel_weight)
        ...
        while len(new_population) < len(population):
            ...
            if random.random() < crossover_rate:
                offspring1, offspring2 = intra_zone_crossover(offspring1, offspring2)
    ...
    final_fitness_scores = evaluate_population(population, data_travel, filtered_precedence_matrix_by_station, station_code, similarity_weight, travel_weight)
    ...
    evaluation, similarity, distance = best_fitness[0], best_fitness[1], best_fitness[2]
...
def create_output_directory(base_dir, similarity_weight, travel_weight):
    # Create a 'results' folder in the base directory
    ...
    # Create a unique directory for the current weight combination inside 'results'
    folder_name = f"sim_weight_{similarity_weight}_travel_weight_{travel_weight}"
...
if __name__ == "__main__":
    ...
    similarity_weights = [0.73]
    travel_weights = [0.27]

    for similarity_weight in similarity_weights:
        for travel_weight in travel_weights:
            counter = 0
            for route_id, route_details in routes.items():
                t2 = time.time()
                best_solution = Main(route_details, travel[route_id], population_size=30, num_generations=300, mutation_rate=0.4, crossover_rate=1, time_limit=6.5, debug=counter, similarity_weight=similarity_weight, travel_weight=travel_weight)
            ...
            output_dir = create_output_directory(base_dir, similarity_weight, travel_weight)

            output_path = path.join(output_dir, 'proposed_sequences.json')
